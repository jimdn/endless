...
func NewServer(addr string, handler http.Handler) (srv *endlessServer) {
	...
	srv.BeforeBegin = func(addr string) {
		Log.Warnln(syscall.Getpid(), addr)
	}
...
func (srv *endlessServer) Serve() (err error) {
	defer Log.Warnln(syscall.Getpid(), "Serve() returning...")
	srv.setState(STATE_RUNNING)
	err = srv.Server.Serve(srv.EndlessListener)
	Log.Warnln(syscall.Getpid(), "Waiting for connections to finish...")
	srv.wg.Wait()
...
func (srv *endlessServer) ListenAndServeTLS(certFile, keyFile string) (err error) {
	...
	Log.Warnln(syscall.Getpid(), srv.Addr)
	return srv.Serve()
}
...
func (srv *endlessServer) getListener(laddr string) (l net.Listener, err error) {
	if srv.isChild {
		...
		if len(socketPtrOffsetMap) > 0 {
			...
			// Log.Warnln("laddr", laddr, "ptr offset", socketPtrOffsetMap[laddr])
		}
...
func (srv *endlessServer) handleSignals() {
	...
	for {
		...
		switch sig {
		case syscall.SIGHUP:
			Log.Warnln(pid, "Received SIGHUP. forking.")
			err := srv.fork()
			if err != nil {
				Log.Warnln("Fork err:", err)
			}
		case syscall.SIGUSR1:
			Log.Warnln(pid, "Received SIGUSR1.")
		case syscall.SIGUSR2:
			Log.Warnln(pid, "Received SIGUSR2.")
			srv.hammerTime(0 * time.Second)
		case syscall.SIGINT:
			Log.Warnln(pid, "Received SIGINT.")
			srv.shutdown()
		case syscall.SIGTERM:
			Log.Warnln(pid, "Received SIGTERM.")
			srv.shutdown()
		case syscall.SIGTSTP:
			Log.Warnln(pid, "Received SIGTSTP.")
		default:
			Log.Warnf("Received %v: nothing i care about...\n", sig)
		}
...
func (srv *endlessServer) shutdown() {
	...
	if err != nil {
		Log.Warnln(syscall.Getpid(), "Listener.Close() error:", err)
	} else {
		Log.Warnln(syscall.Getpid(), srv.EndlessListener.Addr(), "Listener closed.")
	}
}
...
func (srv *endlessServer) hammerTime(d time.Duration) {
	...
	time.Sleep(d)
	Log.Warnln("[STOP - Hammer Time] Forcefully shutting down parent")
	for {
